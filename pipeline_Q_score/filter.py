...
def main() -> None:
    ...
    print(f"DOMAIN: {detected_domain} (from {cfg.KO_JSON}, {cfg.EN_JSON})")
    ...
    if cfg.LIMIT:
        ...
        print(f"{len(keys)} samples from {len(all_keys)} total")
    ...
    if force_fresh:
        print("FORCE_FRESH mode")

    e_src, e_mt, cache_hit = load_cos_cache(cache_key)
    
    if cache_hit and not force_fresh:
        print("cache found for embeddings")
        cos = (e_src * e_mt).sum(1)
    else:
        print("--------------------------------- Computing Cos Similarity ---------------------------------")
        cosModel = SentenceTransformer(cfg.COS_MODEL).to(device)

        e_src = cosModel.encode(src, batch_size=128, normalize_embeddings=True, show_progress_bar=True)
        e_mt  = cosModel.encode(mt , batch_size=128, normalize_embeddings=True, show_progress_bar=True)
        cos   = (e_src * e_mt).sum(1)
    ...
    if hasattr(cfg, 'TERMBASE_PATH') and Path(cfg.TERMBASE_PATH).exists():
        ...
        print("Termbase not found, skip for now")
