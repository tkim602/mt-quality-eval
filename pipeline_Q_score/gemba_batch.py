...
import cfg
from q_score import process_q_score_grading, save_q_score_stats
...
def convert_numpy_types(v):
    if isinstance(v, dict):
        return {k: convert_numpy_types(val) for k, val in v.items()}
    elif isinstance(v, list):
        return [convert_numpy_types(item) for item in v]
...
def _parse(text: str, n: int):
    ...
    if result is None:
        logger.warning(f"Parsing failed: {text[:200]}...")
    else:
...
async def _ask(msgs: List[dict], retry: int = 10):
    for attempt in range(retry):
        try:
            ...
            logger.error(f"ERROR in GEMBA API calls:: {e}")
...
async def gemba_batch(src_texts: List[str], target_texts: List[str]) -> List[dict]:
    ...
    if len(src_texts) != len(target_texts):
        raise ValueError("len(src) != len(tgt)")
...
def _decide_q_score(record: dict, q_processed_data: dict):
    key = record.get("key", "")
    
    for q_rec in q_processed_data:
        if q_rec.get("key") == key:
            tag = q_rec.get("tag", "fail")
            
            passed = []
            failed = []
            
            if q_rec.get("z_cos", 0) > 0:
                passed.append("cosine")
            else:
                failed.append("cosine")
                
            if q_rec.get("z_comet", 0) > 0:
                passed.append("comet")
            else:
                failed.append("comet")
                
            if q_rec.get("z_gemba", 0) > 0:
                passed.append("gemba")
            else:
                failed.append("gemba")
    ...
    return "fail", [], ["cosine", "comet", "gemba"]
...
def _ordered(rec: dict, tag: str, passed: List[str], failed: List[str]) -> dict:
    ...
    ordered_rec = {
        ...
        "mt": rec["mt"],
        "bucket": rec["bucket"],
        "cos": convert_numpy_types(rec["cos"]),
        "comet": convert_numpy_types(rec["comet"]),
        "gemba": convert_numpy_types(rec["gemba"]),
        "gemba_adequacy": convert_numpy_types(rec["gemba_adequacy"]),
        "gemba_fluency": convert_numpy_types(rec["gemba_fluency"]),
        "tag": tag,
        "flag": {"passed": passed, "failed": failed, "gemba_reason": rec.pop("_ev", "")},
        "validation": validation_data,
        "domain": rec.get("domain", "sparrow"), 
    }
    
    for field in ["q_score", "z_cos", "z_comet", "z_gemba", "temp_grade", "final_temp_grade", "q_score_info"]:
        if field in rec:
            ordered_rec[field] = convert_numpy_types(rec[field])
...
async def main():
    ...
    if cfg.USE_Q_SCORE:
        logger.info("----------------------------- Q-Score Processing -----------------------------")
        
        q_processed_data = process_q_score_grading(raw)
        
        for rec in raw:
            q_rec = next((q for q in q_processed_data if q.get("key") == rec.get("key")), None)
            ...
            if q_rec:
                ...
                z_gemba = q_rec.get("z_gemba", 0)
                if z_gemba is not None and z_gemba > 0:
                    passed.append("gemba")
                else:
                    failed.append("gemba")
                
                rec.update({
                    "q_score": convert_numpy_types(q_rec.get("q_score")),
                    "z_cos": convert_numpy_types(q_rec.get("z_cos")),
                    "z_comet": convert_numpy_types(q_rec.get("z_comet")), 
                    "z_gemba": convert_numpy_types(q_rec.get("z_gemba")),
                    "temp_grade": q_rec.get("temp_grade"),
                    "final_temp_grade": q_rec.get("final_temp_grade"),
                    "q_score_info": convert_numpy_types(q_rec.get("q_score_info"))
                })
            else:
                tag, passed, failed = "fail", [], ["cosine", "comet", "gemba"]
            
            final.append(_ordered(rec, tag, passed, failed))
        save_q_score_stats(q_processed_data, out_path)
        
    else:
        logger.error("Q-score in disabled!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        logger.error("set cfg.USE_Q_SCORE = TRUE !!!!!!!!!!!!!!!!!!!!!!!!!")
        raise ValueError("Enable Q-score!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
