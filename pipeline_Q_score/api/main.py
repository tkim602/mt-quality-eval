...
def compute_global_stats(data: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    ...
    for metric in ['cos', 'comet', 'gemba']:
        values = [record[metric] for record in normal_data if record.get(metric) is not None]
        if values:
            stats[metric] = {
...
def calculate_weighted_q_score(gemba: float, comet: float, cos: float, gemba_reason: str = None) -> float:
    ...
    z_cos = (cos - global_stats['cos']['mean']) / global_stats['cos']['std']
    z_comet = (comet - global_stats['comet']['mean']) / global_stats['comet']['std']
...
async def lifespan(app: FastAPI):
    global evaluation_data, global_stats
    try:
...
async def get_analytics():
    ...
    return {
        ...
        "ape_effectiveness": {
            ...
            "avg_q_score_improvement": statistics.mean(ape_improvements["delta_q_score"]) if ape_improvements["delta_q_score"] else 0,
